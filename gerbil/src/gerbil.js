...
(function(w) {
    "use strict";
    var util = {
        getGerbils: function(script_name) {
            var scripts = document.getElementsByTagName("script");
            var gerbils = [];
            for (var i = 0; i < scripts.length; i++) {
                if ((scripts[i].src.indexOf(script_name) > -1) && (scripts[i].id === "")) {
                    gerbils.push(scripts[i]);
                }
            }
            return gerbils;
        },
        getScriptParameters: function(script_url) {
            var pa = script_url.split("?").pop().split("&");
            var p = {};
    ...
    var gerbils = util.getGerbils('gerbil');
    for (var k in gerbils) {
        var url = gerbils[k].src;
        var gerbilid = util.randomString(8);
        gerbils[k].id = "gerbil-"+gerbilid;
        var parsed_params = util.getScriptParameters(url);
        parsed_params.gerbilid = gerbilid;
