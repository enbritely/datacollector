var _scq = _scq || [];
_scq.push(['setMerchantId', '1D6B8E82D9350744']);
...
k)-899497514);j=k;k=e;e=g<<30|g>>>2;g=h;h=c}b[0]=b[0]+h|0;b[1]=b[1]+g|0;b[2]=b[2]+e|0;b[3]=b[3]+k|0;b[4]=b[4]+j|0},_doFinalize:function(){var f=this._data,e=f.words,b=8*this._nDataBytes,h=8*f.sigBytes;e[h>>>5]|=128<<24-h%32;e[(h+64>>>9<<4)+14]=Math.floor(b/4294967296);e[(h+64>>>9<<4)+15]=b;f.sigBytes=4*e.length;this._process();return this._hash},clone:function(){var e=j.clone.call(this);e._hash=this._hash.clone();return e}});e.SHA1=j._createHelper(m);e.HmacSHA1=j._createHmacHelper(m)})();
  ...
  return {
    ...
    isArray: isArray,
    playQueue: playQueue,
    inspectorSrc: ('https:' === document.location.protocol ? 'https://recommender' : 'http://cdn') + '.scarabresearch.com/static/inspector/scarab-inspector.min.js'
  };
}();
'use strict';
...
var ScarabModule = function(win, config) {
    ...
    var addEvent = function(o, name, fn, ctx) {
        ...
        if (o.addEventListener) {
            var evl = ScarabUtil.bind(fn, ctx);
    ...
    var Feature = function(config) {
        ...
        this.parent = this.containerId ? document.getElementById(this.containerId) : null; // parentElement
    ...
    Transaction.prototype = {
        ...
        serializeContext : function() {
                    ...
                    if (!feature.t && feature.f && feature.f.indexOf(key) === 0){
                        ...
                        if (ft && typeof ft.validate === 'function' && !ft.validate(tr)) {
                            if (console && console.error) {
                                console.error('no "' + ft.trigger + '" trigger for "' + feature.f + '" feature.');
                            }
                        }
                    }
                }
            ...
            if (availabilityZone) {
                slist.push('az=' + encodeURIComponent(availabilityZone));
            }
            ...
            if (data.features) {
                for (feature in data.features) {
                    if (data.features.hasOwnProperty(feature)) {
                        currentFeature = this.findFeature(feature);
                        if (currentFeature) {
    ...
    var setAvailabilityZone = function(az) {
        availabilityZone = az;
    };

    var setEmail = function(email) {
        if (!email) return;
        var normalized = ScarabUtil.trim(email).toLowerCase();
        if (!normalized) return;
        setEmailHash(ScarabUtil.sha1(normalized).substring(0,16) + '1');
    };
    ...
    var publicInterface = {
        ...
        cart : function(cartArray) {
            var tr = getCurrentTransaction();
            var c = [];
            for (var i = 0, l = cartArray.length; i < l; i++) {
                if ( typeof cartArray[i].item === 'undefined') {
                    if (console && console.error) {
                        console.error('missing "item" in cart command', cartArray[i]);
                    }
                    return;
                }
                c.push(new Item({
        ...
        purchase: function(conf){
            ...
            conf = conf || {};
            var config = {};
            if (typeof conf.items === 'undefined') {
                if (console && console.error) {
                    console.error('No items specified in purchase', conf);
                }
                return;
            }
            if (typeof conf.orderId === 'undefined') {
                if (console && console.warn) {
                    console.warn('No orderId specified in purchase', conf);
                }
            }
            for (var key in conf){
                if (key === 'orderId') {
                     if (typeof conf.orderId === 'string' || typeof conf.orderId === 'number') {
                        config.orderId = conf.orderId.toString();
                    } else {
                        if (console && console.warn) {
                            console.warn('Invalid orderId in purchase (should be a string or number)', conf.orderId);
                        }
                    }
                } else if (key === 'items') {
                    if (ScarabUtil.isArray(conf.items)) {
                        if (conf.items.length === 0) {
                            if (console && console.error) {
                                console.error('No items in purchase (empty array)', conf);
                            }
                            return;
                        }
                        config.items = [];
                        ...
                        if (console && console.warn) {
                            console.warn('Invalid items in purchase (should be an array)', conf.items);
                        }
                        return;
                    }
                } else {
...
(function() {
    ...
    try {
        var qInspector = ScarabUtil.parseQueryString()['sc_inspector'];
        if (qInspector === 'true') {
            var myDate = new Date();
            myDate.setMinutes(myDate.getMinutes() + 30);
            document.cookie = 'sc_inspector=true;expires=' + myDate + ';path=/';
        } else if (qInspector === 'false') {
            var myDate = new Date();
            myDate.setMinutes(myDate.getMinutes() - 30);
            document.cookie = 'sc_inspector=false;expires=' + myDate + ';path=/';
        }
        if (ScarabUtil.getCookie('sc_inspector') === true) {
            var d = document, t = 'script',
                o = d.createElement(t),
                s = d.getElementsByTagName(t)[0];
            o.src = ScarabUtil.inspectorSrc;
